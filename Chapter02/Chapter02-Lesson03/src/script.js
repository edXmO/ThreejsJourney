...
bushTwo.position.set(1.4, 0.2, 2.2);

const bushThree = new THREE.Mesh(bushGeometry, bushMaterial);
bushThree.scale.set(0.5, 0.5, 0.5);
bushThree.position.set(-0.8, 0.2, 2.2);

const bushFour = new THREE.Mesh(bushGeometry, bushMaterial);
bushFour.scale.set(0.25, 0.25, 0.25);
bushFour.position.set(-1.4, 0.2, 2.2);
...
{   
    ...
    const x = Math.sin(randAngle) * radius;
    const z = Math.cos(randAngle) * radius;
...
const tick = () =>
{
    ...
    ghost3.position.y = Math.sin(elapsedTime * 5) -  Math.sin(elapsedTime * 1.5)
